...
export const addClothes = async (data) => {
  const response = await client.post('/clothes/me', data)
  return response.data
}

// 개별 의류 정보 조회
export const getClothesDetail = async (fashionItemId) => {
  const response = await client.get(`/clothes/${fashionItemId}`)
  return response.data
}

// 개별 의류 정보 수정
export const updateClothes = async (fashionItemId, data) => {
  const response = await client.patch(`/clothes/${fashionItemId}`, data)
  return response.data
}
